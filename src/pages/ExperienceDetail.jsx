...
export default function ExperienceDetail() {
  ...
  const [community, setCommunity] = useState(null) // { avg_rating, total_ratings }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [{ data: expData, error: expError }, { data: ueData }, { data: avgData }] =
        await Promise.all([
  ...
  // ── Persist helper — always upsert so it works for first-time interactions ──
  ...
  const toggleStatus = () => {
    // Cancel any pending stepper save — this write takes precedence
    clearTimeout(visitTimerRef.current)
    const today = new Date().toISOString().split('T')[0]

    if (userExp?.completed) {
      // Mark as not done → reset visit count to 0
      ...
      // Mark as done → bump visit count to at least 1
  ...
  const changeVisits = (delta) => {
    ...
    // Sync completed when crossing the 0 boundary
    let extra = {}
    if (next === 0 && prev > 0) {
      extra = { completed: false, last_visited_date: null }
    } else if (next === 1 && prev === 0) {
      extra = { completed: true, last_visited_date: today }
    }

    setUserExp(prev => ({ ...(prev ?? {}), times_visited: next, ...extra }))

    // Debounce DB write so rapid taps send one call
    clearTimeout(visitTimerRef.current)
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      navigator.share({ title: exp.name, text }).catch(() => {})
    } else {
    ...
    <div className="flex flex-col bg-gray-50 min-h-full">
      ...
      <div className="flex flex-col gap-4 px-4 py-4 pb-8">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          {/* Pills row 1 — park / category / type */}
...
function SectionLabel({ children }) {
  return (
    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">{children}</p>
  )
}
